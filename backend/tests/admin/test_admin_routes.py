...
@pytest.mark.asyncio
async def test_approve_user_kyc_success(mock_approve_kyc, override_admin_user, override_get_db, async_client):
...
@patch.object(AdminService, 'approve_kyc', new_callable=AsyncMock)
@pytest.mark.asyncio
async def test_kyc_user_not_found(mock_approve_kyc, override_admin_user, override_get_db, async_client):
...
@pytest.mark.asyncio
async def test_reject_user_kyc_success(mock_reject_kyc, override_admin_user, override_get_db, async_client):
...
@pytest.mark.asyncio
async def test_freeze_user_account(mock_freeze_user, override_admin_user, override_get_db, async_client):
...
@pytest.mark.asyncio
async def test_unfreeze_user_account(mock_unfreeze_user, override_admin_user, override_get_db, async_client):
...
@pytest.mark.asyncio
async def test_unban_user_account(mock_unban_user, override_admin_user, override_get_db, async_client):
...
@pytest.mark.asyncio
async def test_delete_user_account(mock_delete_user, override_admin_user, override_get_db, async_client):
...
@pytest.mark.asyncio
async def test_delete_flagged_review_twice(mock_delete_review, override_admin_user, override_get_db, async_client):
    """Test deleting flagged review and attempting deletion again (404)."""
    review_id = uuid4()
    mock_delete_review.side_effect = [None, HTTPException(status_code=404, detail="Review not found")]
    async with async_client as ac:
